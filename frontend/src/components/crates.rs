...
pub fn CratesPanel(props: &CratesPanelProps) -> Html {
    ...
    html! {
        <div class="relative">
            <button onclick={toggle} class="p-3 text-sm cursor-pointer bg-gray-800 rounded-md shadow-lg text-gray-400 hover:bg-gray-900 flex items-center gap-1">
                {"Crates"}
                <span class="text-xs">{if *open { "\u{25b2}" } else { "\u{25bc}" }}</span>
            </button>
            if *open {
                <div class="absolute right-0 top-full mt-1 z-50 bg-gray-800 border border-gray-600 rounded-md shadow-xl p-3 min-w-[200px]">
                    ...
                    <div class="flex flex-col gap-1">
                        { for deps.iter().map(|(name, ver)| html! {
